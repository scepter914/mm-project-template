...
class Waymo2KITTI(object):
    ...
    def convert(self):
        ...
        if self.workers == 0:
            data_infos = mmengine.track_progress(self.convert_one, range(len(self)))
        else:
        ...
        for c in calibrations:
            ...
            range_image_cartesian = (
                range_image_utils.extract_point_cloud_from_range_image(
                    ...
                    tf.expand_dims(
                        tf.convert_to_tensor(value=beam_inclinations), axis=0
                    ),
    ...
    def create_waymo_info_file(self, frame, file_idx, frame_idx, file_infos):
        ...
        # Gather frame infos
        sample_idx = f"{self.prefix}{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}"
        frame_infos["sample_idx"] = int(sample_idx)
        frame_infos["timestamp"] = frame.timestamp_micros
        frame_infos["ego2global"] = (
            np.array(frame.pose.transform).reshape(4, 4).astype(np.float32).tolist()
        )
        ...
        for i, (cam_key, camera_calib, Tr_velo_to_cam) in enumerate(
            ...
            cam_infos["lidar2img"] = (
                (camera_calib @ Tr_velo_to_cam).astype(np.float32).tolist()
            )
        ...
        if not self.test_mode:
            ...
            if self.save_cam_instances:
                frame_infos["cam_instances"] = self.gather_cam_instance_info(
                    copy.deepcopy(frame_infos["instances"]), frame_infos["images"]
                )
        ...
        for obj in frame.laser_labels:
            ...
            if cam_sync:
                if obj.most_visible_camera_name:
                    name = self.cam_list.index(f"_{obj.most_visible_camera_name}")
            ...
            instance_info["bbox_3d"] = (
                np.array([x, y, z, length, width, height, rotation_y])
                .astype(np.float32)
                .tolist()
            )
    ...
    def gather_cam_instance_info(self, instances: dict, images: dict):
        ...
        for cam_type in self.camera_types:
            ...
            for instance in instances:
                ...
                gt_bboxes_3d = LiDARInstance3DBoxes(gt_bboxes_3d[None, :]).convert_to(
                ...
                corner_coords = view_points(corners_3d, cam2img, True).T[:, :2].tolist()
                ...
                cam_instance["bbox_3d"] = (
                    gt_bboxes_3d.numpy().squeeze().astype(np.float32).tolist()
                )
    ...
    def merge_trainval_infos(self):
        ...
        mmengine.dump(
            trainval_infos,
            osp.join(
                osp.dirname(self.save_dir), f"{self.info_prefix}_infos_trainval.pkl"
            ),
        )
